...
def split_multibrand_block(text: str, target_brand: str, brand_set: set[str]) -> list[str]:
    """
    Given a text block that might contain multiple brand names, return only segments
    that belong to the target brand. We split on occurrences of *other* brands.
    """
    tb = (target_brand or "").lower().strip()
    if not tb:
        return [text]

    # Build regex of other brands (escape, sort by length desc to match longer first)
    others = [b.lower() for b in brand_set if b and b.lower() != tb]
    if not others:
        return [text]
    others_sorted = sorted(set(others), key=len, reverse=True)
    # Avoid pathological regex if there are odd characters
    parts_regex = r"(" + "|".join(re.escape(b) for b in others_sorted) + r")"

    try:
        # Split on other brand tokens; keep pieces that contain the target brand only
        raw_parts = re.split(parts_regex, text, flags=re.I)
    except re.error:
        raw_parts = [text]
    ...
    for part in raw_parts:
        if not part or part.isspace():
            continue
        pl = part.lower()
        if tb in pl and not any(ob in pl for ob in others_sorted):
